...
SAMPLE_RATE = 2000
FIELD_dat = "Wall_heat_flux"
FIELD_pickle = "FLUX_wall"
FILENAME = "WHF"
T_START1 = 0.31
N_PERIOD = 5
...
def period(plot=True):
    ...
    f_max, amp_max = maximums(freq_plot, PSD_plot, max_number=60)
    print(f_max)
...
theta = wall[wall.key_list[200]]["theta_wall"]
time_avg_flux = np.zeros(len(theta))


for i in range(idx_start+1,idx_end+1):
    dt = time[i]-time[i-1]
    time_avg_flux += -wall[wall.key_list[i]][FIELD_pickle]*dt
...
for i in range(idx_start+1,idx_end+1):
    dt = time[i] - time[i - 1]
    time_std_flux += (-wall[wall.key_list[i]][FIELD_pickle]-time_avg_flux)**2*dt
