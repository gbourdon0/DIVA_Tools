...
SAMPLE_RATE = 2000
FIELD_dat = "Interface_gas_heat_flux"
FIELD_pickle = "Film thickness"
FILENAME = "FT"
T_START1 = 0.3
N_PERIOD = 10
...
def period(plot=True):
    ...
    f_max, amp_max = maximums(freq_plot, PSD_plot, max_number=150)
    print(f_max)
...
if DEBUG:
    ...
    plt.vlines(T_START,y_min,y_max, color = "k")
    plt.vlines(T_END,y_min,y_max,color = "k")
    plt.show()
DT = T_END - T_START
print(
...
ax1.plot(theta,time_avg_flux*1000, label = r"$\mu$")
ax1.fill_between(theta, y1=(time_avg_flux + time_std_flux)*1000, y2=(time_avg_flux - time_std_flux)*1000 , alpha=0.5, color="r",label=r"$\mu$ +/- $\sigma$")
ax1.set_xlim(0,180)
ax1.set_xlabel(r"$\theta$ (deg)")
ax1.set_ylabel(r"Film Thickness (mm)")
ax1.grid(visible=True)
plt.legend()
