...
def wall_heat_flux2(Tecplot_obj, Diva_input_obj, frame, phi_sol_name, phi_liq_name):
    """
    Compute the wall heat fluw without interpolating in the exact location of phi_sol = 0
    :param phi_liq_name: name of the phi function for the fluid region
    ...
    :return: Heat flux along the interface considering the interface to be in the center of the cell where phi near
    0 to the cell
    """
    # Variable reshapping
    ...
    gradT_x = Tecplot_obj[frame]["\/Tghost_immersed_sol_x"].reshape(Tecplot_obj[1].mesh_dim[0],
                                                                    Tecplot_obj[1].mesh_dim[1])
    gradT_y = Tecplot_obj[frame]["\/Tghost_immersed_sol_y"].reshape(Tecplot_obj[1].mesh_dim[0],
                                                                    Tecplot_obj[1].mesh_dim[1])
    flux = []
    for i in range(Tecplot_obj[1].mesh_dim[0]):
        for j in range(Tecplot_obj[1].mesh_dim[1]):
            if abs(phi_sol[i][j]) <= Tecplot_obj.dx[i + j] / 2 or abs(phi_sol[i][j]) <= Tecplot_obj.dy[i + j] / 2:
                if phi_liq[i][j] > 0:
                ...
                flux.append(np.nan)
    Tecplot_obj[frame]["Wall Heat Flux"] = flux


def wall_heat_flux(Tecplot_obj, Diva_input_obj, frame, phi_sol_name, phi_liq_name):
    """
    Compute the wall heat flux with interpolating in the exact location of phi_sol = 0
    :param phi_liq_name: name of the phi function for the fluid region
    :param phi_sol_name: name of the phi function for the IMB
    :param frame: number of the frame for Tecplot_obj
    :param Diva_input_obj: Input Diva obj as defined in diva_input
    ...
    :return: Heat flux along the interface considering the interface to be in the center of the cell where phi near
    0 to the cell
    :return: Heat flux along the intefrace considering the interface to be in the center of the cell where phi near
    0 to the cell
    """
    # Variable reshapping

    phi_liq =  Tecplot_obj.get_grid_data(frame=frame, var_name=phi_liq_name)
    phi_nx =  Tecplot_obj.get_grid_data(frame=frame, var_name=phi_sol_name + "_nx")
    phi_ny =  Tecplot_obj.get_grid_data(frame=frame, var_name=phi_sol_name + "_ny")


    gradT_x =  Tecplot_obj.get_grid_data(frame=frame, var_name="\/Tghost_immersed_sol_x")
    gradT_y =  Tecplot_obj.get_grid_data(frame=frame, var_name="\/Tghost_immersed_sol_y")
    ...
    Tecplot_obj[frame]["Wall Heat Flux_field"] = flux
    return None

def old_wall_heat_flux(Tecplot_obj, Diva_input_obj, frame, phi_sol_name, phi_liq_name):
    """
    Compute the wall heat flux with interpolating in the exact location of phi_sol = 0
    :param phi_liq_name: name of the phi function for the fluid region
    :param phi_sol_name: name of the phi function for the IMB
...
def old_angular_wall_heat_flux(Tecplot_obj, frame, phi_sol_name):
    """
    ...
    '''grid_x, grid_y = xx, yy
    points = np.array([x, y])
    values = Tecplot_obj[frame]["Wall Heat Flux_field"]
    ...
    from modules.post_proc_2D.interp2D import interp2D
    FLUX = interp2D(np.array(Tecplot_obj[frame]["Wall Heat Flux_field"]), x, y, xx, yy, resolve_nan = True,kind = 'cubic')
    #FLUX = interp2D(np.array(Tecplot_obj[frame]["Wall Heat Flux_field"]), x, y, xx, yy)
...
def angular_wall_heat_flux(Tecplot_obj, frame, phi_sol_name):
    ...
    #interpolating
    FLUX = interp2D(np.array(df_reduce["Wall Heat Flux_field"]), x, y, xx, yy, resolve_nan = True,kind = 'linear')
