...
def interface_liq_heat_flux(Tecplot_obj, Diva_input_obj, frame, phi_liq_name):
    """
    ...
    gradT_x = Tecplot_obj.get_grid_data(frame=frame, var_name="\/T_liq_x")
    gradT_y = Tecplot_obj.get_grid_data(frame=frame, var_name="\/T_liq_y")

    kappa = np.ones(gradT_x.shape)*Diva_input_obj.TP.kth_liq

    flux = -kappa * (gradT_x*phi_nx + gradT_y*phi_ny)
    flux = flux.reshape(flux.size)

    Tecplot_obj[frame]["Interface Liq Heat Flux_field"] = flux
    return None


def old_interface_liq_heat_flux(Tecplot_obj, Diva_input_obj, frame, phi_liq_name):
    """
    ...
    gradT_x = Tecplot_obj[frame]["\/T_liq_x"].reshape(Tecplot_obj[1].mesh_dim[0], Tecplot_obj[1].mesh_dim[1])
    gradT_y = Tecplot_obj[frame]["\/T_liq_y"].reshape(Tecplot_obj[1].mesh_dim[0], Tecplot_obj[1].mesh_dim[1])
    flux = []
...
def old_angular_interface_liq_heat_flux(Tecplot_obj, frame, phi_liq_name, phi_sol_name):
    """
    ...
    grid_x, grid_y = xx,yy
    points = np.array([x,y])
    values = Tecplot_obj[frame]["Interface Liq Heat Flux_field"]
...
def angular_interface_liq_heat_flux(Tecplot_obj, frame, phi_liq_name,phi_sol_name):
    """
    Compute the angular heat flux from gas to liquid (without the evaporation rate)
    ...
    #interpolating
    FLUX = interp2D(np.array(df_reduce["Interface Liq Heat Flux_field"]), x, y, xx, yy, resolve_nan = True,kind = 'linear')
