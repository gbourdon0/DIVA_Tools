...
class Temperature_Boundary_Condition:
    def __init__(self):
        # Type of conditions
        self.BCX1 = None
        self.BCXN = None
    ...
    def read_file(self, path):
        ...
        :return: None
        """
        f = open(path + "/Input_File_Temperature_Boundary_Condition", "r")
        files = f.readlines()
        f.close()
        files = iter(files)
        for lines in files:
            ...
            if "*TYPE OF BOUNDARY CONDITIONS" in lines:
                for i in range(9):
                    next(files)
                self.BCX1 = dico.BC_temperature[self.get_value(next(files), 1, v_type=int)]
                self.BCXN = dico.BC_temperature[self.get_value(next(files), 1, v_type=int)]
                self.BCY1 = dico.BC_temperature[self.get_value(next(files), 1, v_type=int)]
                self.BCYN = dico.BC_temperature[self.get_value(next(files), 1, v_type=int)]
                self.BCZ1 = dico.BC_temperature[self.get_value(next(files), 1, v_type=int)]
                self.BCZN = dico.BC_temperature[self.get_value(next(files), 1, v_type=int)]
    ...
    def get_X1_BC(self, rtype=tuple):
        ...
        if rtype == str:
            ...
            if self.BCX1 == dico.BC_temperature[1]:
                out += dico.BC_temperature[1] + ", " + str(self.BCX1_dir) + " K"
            elif self.BCX1 == dico.BC_temperature[3]:
                out += dico.BC_temperature[3] + ", " + str(self.BCX1_neu) + " W/m2"
            else:
    ...
    def get_XN_BC(self, rtype=tuple):
        ...
        if rtype == str:
            ...
            if self.BCXN == dico.BC_temperature[1]:
                out += dico.BC_temperature[1] + ", " + str(self.BCXN_dir) + " K"
            elif self.BCXN == dico.BC_temperature[3]:
                out += dico.BC_temperature[3] + ", " + str(self.BCXN_neu) + " W/m2"
            else:
                raise NotImplemented(
                    "For this choice, function are not implement yet. Only uniform Neumann and Dirichlet are")
            return out
    ...
    def get_Y1_BC(self, rtype=tuple):
        ...
        if rtype == str:
            ...
            if self.BCY1 == dico.BC_temperature[1]:
                out += dico.BC_temperature[1] + ", " + str(self.BCY1_dir) + " K"
            elif self.BCY1 == dico.BC_temperature[3]:
                out += dico.BC_temperature[3] + ", " + str(self.BCY1_neu) + " W/m2"
            else:
                raise NotImplemented(
                    "For this choice, function are not implement yet. Only uniform Neumann and Dirichlet are")
            return out
    ...
    def get_YN_BC(self, rtype=tuple):
        ...
        if rtype == str:
            ...
            if self.BCYN == dico.BC_temperature[1]:
                out += dico.BC_temperature[1] + ", " + str(self.BCYN_dir) + " K"
            elif self.BCYN == dico.BC_temperature[3]:
                out += dico.BC_temperature[3] + ", " + str(self.BCYN_neu) + " W/m2"
            else:
                raise NotImplemented(
                    "For this choice, function are not implement yet. Only uniform Neumann and Dirichlet are")
            return out
    ...
    def get_Z1_BC(self, rtype=tuple):
        ...
        if rtype == str:
            ...
            if self.BCZ1 == dico.BC_temperature[1]:
                out += dico.BC_temperature[1] + ", " + str(self.BCZ1_dir) + " K"
            elif self.BCZ1 == dico.BC_temperature[3]:
                out += dico.BC_temperature[3] + ", " + str(self.BCZ1_neu) + " W/m2"
            else:
                raise NotImplemented(
                    "For this choice, function are not implement yet. Only uniform Neumann and Dirichlet are")
            return out
    ...
    def get_ZN_BC(self, rtype=tuple):
        ...
        if rtype == str:
            ...
            if self.BCZN == dico.BC_temperature[1]:
                out += dico.BC_temperature[1] + ", " + str(self.BC_ZN_dir) + " K"
            elif self.BCZN == dico.BC_temperature[3]:
                out += dico.BC_temperature[3] + ", " + str(self.BC_ZN_neu) + " W/m2"
            else:
                raise NotImplemented(
                    "For this choice, function are not implement yet. Only uniform Neumann and Dirichlet are")
            return out
